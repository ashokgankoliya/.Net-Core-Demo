...
Partial Class UploadGrowerFarmFiles
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If file IsNot Nothing AndAlso file.ContentLength Then
                ...
                Dim HeaderID As String = Request.Form("hdnfarmheaderid").ToString()
                Dim FarmCode As String = Request.Form("hdnfarmcode").ToString()
                Dim DetailID As String = Request.Form("hdnfarmdetailid").ToString()
                Dim MainFolderName = String.Format(ConfigurationManager.AppSettings("UserUploadPath"))
                Dim SubFolderName = String.Format(ConfigurationManager.AppSettings("GrowerReportFarmPath"), DetailID)
                ...
                Dim NewFileName As String = fname.Replace(FileType, "") + "_" + HeaderID + "_" + FarmCode + FileType
                file.SaveAs(Server.MapPath(Path.Combine("~/" + MainFolderName + "/" + SubFolderName + "", NewFileName)))


                result = "<script language='javascript' type='text/javascript'>window.top.window.stopUploadFarmFiles('" + NewFileName + "','" + FileType + "');</script>"

            Else
                result = "<script language='javascript' type='text/javascript'>window.top.window.stopUploadFarmFiles('-nil-','');</script>"
            End If
