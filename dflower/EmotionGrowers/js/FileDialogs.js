var FileDialogsConfig = function() { return {width : 0, height : 0, padding: 5, save : { image : "", handler : ""}, open : { image : "", handler : ""}}}
var FileDialogs = function (placeHolder, config) {    
	var movieName = "FileDialogs.swf";	
	if (placeHolder == undefined || config == undefined) return;
	...
	if (config.open.image != "" && config.open.handler != "") {
		params += "openImage=" + config.open.image + "&openCallback=" + config.open.handler;
	}
	if (config.save.image != "" && config.save.handler != "") {
		params += ((params.match("&$") == "&") ? "" : "&") + "saveImage=" + config.save.image + "&saveCallback=" + config.save.handler;
	}
	...
	if (navigator.userAgent.match(/MSIE/)) {
		...
		html = "<object classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000' width='500' height='500' id='FileDialogsFlash' data="+ movieName +" type='application/x-shockwave-flash'>" +
		...
		holder.setAttribute("style", holder.getAttribute("style") + "; float:left;");
